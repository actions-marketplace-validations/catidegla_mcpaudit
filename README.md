...
There is no shortage of MCP scanners. [`mcp-guardian`](https://www.npmjs.com/package/mcp-guardian), [`sentinel-scan-cli`](https://www.npmjs.com/package/sentinel-scan-cli), [`mcp-security-scanner`](https://www.npmjs.com/package/mcp-security-scanner), `@kryptosai/mcp-observatory` and Wallarm's `rogue-mcp` all publish one, and several are more actively developed than this. If you want breadth of checks, runtime interception or a hosted dashboard, look at those.

The difference here is what happens on a false alarm. This is the only one of them built around a regression corpus of real, widely installed servers that **must produce no findings**, with CI failing if one does. That constraint is what forces multi-signal scoring instead of keyword matching, and it is the whole design. Everything else follows from refusing to be wrong four times in five.
